...
- 首頁列表：規線分隔的單欄清單（不用卡片），數字用等寬字對齊
- 封面：`aspect-ratio: 3/4`，缺圖時佔位背景色 + 書名首字（JS 設 `onerror` 隱藏 img 顯示佔位）
- 360px 寬無橫向捲軸；字級：內文 16px、行高 1.8
- 配色：依 mono-color 編輯印刷系統，最多兩種墨——紙色底（一般頁中性白 `#FAFAF7`、有生之年區淡米色 `#F5F1E8`）、主墨鈷藍 `#2148B8`（文字、規線、封面、按鈕）、輔墨陶土橙 `#C65F38`（只用於排名訊號：上升箭頭、目前分頁、日期底線、hover 套印偏移）；無漸層、無圓角卡片、無外部字體，封面佔位用 CSS 網點 + 書名首字
