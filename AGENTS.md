...
5. **全篇繁體中文**，不可出現簡體字。唯一例外：`wiki` 欄位與 `global_zh.json` 的 key 是查詢鍵，
   照抄原文（可能是簡體或外文）。
   ⚠️ **日文新字體也要換成繁體正字**，簡體掃描抓不到（見 §10.6）。已知對照（2026-09-12 擴充）：
   `凛→凜 条→條 寿→壽 単→單 鉄→鐵 竜→龍 沢→澤 桜→櫻 歳→歲 剣→劍 弐→貳 蔵→藏`
   `届→屆 触→觸 鴎→鷗 缶→罐 弁→辯 売→賣 険→險 駅→驛 広→廣 弾→彈 体→體 発→發`
   `図→圖 会→會 来→來 価→價 済→濟 両→兩 独→獨 乗→乘 覚→覺 実→實 帰→歸 観→觀`
   `経→經 続→續 戦→戰 芸→藝 医→醫`
   🔴 **「者」不是新字體，是標準繁體字**（作者／讀者），不可誤列進對照表去擋。
...
8. **章節 `title` 的三段優先序**（2026-09-12 第六批確立）：
   ① 有台灣官方中譯 → 用官方的；
   ② 查不到官方中譯 → **自行意譯成繁體中文**，不可留日文原題；
   ③ **作品本身就沒有逐話副標題（官方只有話號）→ `title` 留空字串 `""`**。
   🔴 **第 ③ 種情況絕對不可以填「第N話」**——`assets/js/app.js:224` 的章節列表
   已經自己渲染「第 N 話」，標題再填一次會顯示成「第 1 話　第1話」。
   ⚠️ 例外：**官方標題本身就是拉丁字母時照用**（`one-piece` 第 1 話「ROMANCE DAWN」是既有前例），
   但**同一本書不可體例混用**（一半中文標題、一半英文佔位字串）。
   ⚠️ 標題同樣不可含日文假名與新字體，規則同第 5 條；
   人名分隔用中文間隔號 `·`(U+00B7)，**不可用日文中點 `・`(U+30FB)**。
   📌 **既有違規基準線**（本條建立於 2026-09-12，以下 6 本在那之前就違規，屬待清理項目，
   §12 的標題檢查會自動扣掉它們，所以檢查印出來的一律是新引入的問題）：
   `silver-spoon`／`inuyasha`／`mushishi`（章節標題保留日文原題）、
   `reborn`／`rurouni-kenshin`（單話用英文標題、與同書中文標題混用）、
   `haikyu`（第 400 話用日文中點 `・`）。清理進度見 `handoff/` 最新一份。
...
- **簡體掃描的判準**：「s2t 會變、且 t2s 不會變」再扣白名單。直接用 OpenCC s2t 誤報率極高——
  ...
  2026-09-12 第六批再確認四個誤報，一併加入白名單：
  **准**（批准、准將）、**划**（划船、划算）、**杠**（《地獄樂》角色名）、**朴**（朴朔子）。
...
# 章節標題（§10.2 第 8 條）：不可含假名或日文中點、不可填「第N話」、同書不可體例混用
# ⚠️ 既有 6 本在本檢查建立前就違規（見下方 KNOWN），腳本會自動扣掉它們，
#    所以這裡印出來的一律是「新引入的問題」，不可忽略。要清理 KNOWN 是另一件事。
python3 -c "
import json,re
KANA=re.compile(r'[\u3040-\u309f\u30a0-\u30fa\u30fc-\u30ff\u3005\u3006]')  # 不含 U+30FB・，另行檢查
KNOWN={'silver-spoon','inuyasha','mushishi','reborn','rurouni-kenshin','haikyu'}
bad=[]
for f in ('manga','classics','essentials'):
    for b in json.load(open(f'data/{f}.json')):
        if b['id'] in KNOWN: continue
        for c in b['chapters']:
            t=c['title']
            if KANA.search(t): bad.append((b['id'],c['num'],'含日文假名',t))
            if '\u30fb' in t: bad.append((b['id'],c['num'],'日文中點・應為·',t))
            if re.fullmatch(r'第\s?\d+\s?話',t.strip()): bad.append((b['id'],c['num'],'與介面自帶話號重複，應留空',t))
        ne=[t for t in b['chapters'] if t['title']]
        hc=[bool(re.search(r'[\u4e00-\u9fff]',c['title'])) for c in ne]
        if ne and any(hc) and not all(hc):
            bad.append((b['id'],'-','體例混用',[c['title'] for c,k in zip(ne,hc) if not k]))
print('標題異常:',bad or '無')
"
